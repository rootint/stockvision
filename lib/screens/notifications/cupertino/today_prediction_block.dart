...
class _TickerRowState extends State<TickerRow> {
  ...
  Widget build(BuildContext context) {
    ...
    // sockets
    var currentPrice =
        provider.getPriceData(widget.data.ticker).currentMarketPrice;
    provider.initPriceStream(widget.data.ticker);
    final metadata =
        Provider.of<YahooMetaProvider>(context).getMetaData(widget.data.ticker);
    return SizedBox(
      height: 70,
      child: Row(
        children: [
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Prediction' + ((widget.data.atClose) ? ' at close:' : 'at open:'),
                  ),
                ],
              ),
            ),
